...
export default function UserProfilePage() {
  ...
  const handlePasswordSave = async (e: React.FormEvent) => {
    ...
    if (passwords.newPassword.length < 8) {
      setPwToast({ type: 'error', message: 'Le nouveau mot de passe doit contenir au moins 8 caractères' });
      return;
    }
    setPwLoading(true);
    try {
      ...
      setPwToast({ type: 'success', message: 'Mot de passe changé avec succès !' });
    } catch (err: unknown) {
      setPwToast({ type: 'error', message: err instanceof Error ? err.message : 'Erreur lors du changement' });
    } finally {
      setPwLoading(false);
    }
  };
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <section className="rounded-2xl border border-red-100 bg-white shadow-sm p-6">
        ...
        <p className="text-sm text-slate-600 mb-4 leading-relaxed">
          La suppression de votre compte efface définitivement toutes vos données : stratégies, contenus, calendrier et historique. Cette action ne peut pas être annulée.
        </p>
        <button
          type="button"
      ...
      {deleteOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-2xl bg-white shadow-2xl p-6">
            ...
            <p className="text-sm text-slate-600 mb-4 leading-relaxed">
              Tapez <span className="font-bold text-red-600">SUPPRIMER</span> pour confirmer la suppression définitive de votre compte.
            </p>
