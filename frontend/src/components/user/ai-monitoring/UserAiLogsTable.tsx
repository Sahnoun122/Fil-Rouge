...
interface UserAiLogsTableProps {
  logsResult: UserAiLogsResult | null;
  isLoading?: boolean;
  onPageChange: (nextPage: number) => void;
  onViewDetails: (log: UserAiLog) => void;
}
...
export default function UserAiLogsTable({
  logsResult,
  isLoading = false,
  onPageChange,
  onViewDetails,
}: UserAiLogsTableProps) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-black text-slate-900">My AI Logs</h2>
                ...
                logs.map((log) => (
                  <tr key={log.id} className="align-middle">
                    ...
                    <td className="px-4 py-3">
                      <p className="max-w-[320px] truncate text-slate-700">{log.actionType}</p>
                    </td>
                    <td className="px-4 py-3">
